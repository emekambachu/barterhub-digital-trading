...
class HomeController extends Controller
{
    ...
    public function recoverPassword(Request $request){
        ...
        Mail::send('emails.users.recover-password', $data, static function ($message) use ($data) {
            $message->from('support@barterhub.ltd', 'BarterHub Digital Trading');
            $message->to($data['email'], $data['name']);
            $message->replyTo('support@barterhub.ltd', 'BarterHub Digital Trading');
    ...
    public function contactForm(Request $request){
        ...
        Mail::send('emails.contact-form', $data, static function ($message) use ($data) {
            $message->from('info@bit-farms.ltd', 'BarterHub Digital Trading');
            $message->to('support@barterhub.ltd', 'BarterHub Digital Trading');
            $message->replyTo('noreply@bit-farms.ltd', 'BarterHub Digital Trading');
